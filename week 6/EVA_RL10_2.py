...
#        self.food = self.canvas.create_image(1510, 10, anchor='nw', image = food_file)
        ...
        self.mouse = self.canvas.create_oval(10 + self.start[0]*self.size, 10 + self.start[1]*self.size, 10 + self.start[0]*self.size + self.size - 20, 10 +self.start[1]*self.size + self.size - 20, fill = 'gray')
#       self.barriers = self.canvas.create_rectangle(10 + self.barriers[0][0]*self.size, 10 + self.barriers[0][1]*self.size,(self.barriers[0][0] + 1)*self.size - 10, (self.barriers[0][1] + 1)*self.size - 10,fill = 'gray')
        ###################
        beta = (self.distance(self.goals[0], self.fake[0]) + self.distance(self.start, self.goals[0]) - self.distance(self.start, self.fake[0])) / 2
        self.LDP = []
        for a in range(9):
            for b in range(9):
                ldp = [a, b]
                if (self.distance(ldp, self.goals[0]) == beta) and (self.distance(ldp, self.fake[0]) == self.distance(self.goals[0], self.fake[0]) - self.distance(ldp, self.goals[0])):
                    self.LDP.append(ldp)
        for i in range(len(self.LDP)):
            self.LDP[i] = self.canvas.create_rectangle(10 + self.LDP[i][0]*self.size, 10 + self.LDP[i][1]*self.size,(self.LDP[i][0] + 1)*self.size - 10, (self.LDP[i][1] + 1)*self.size - 10,fill = 'green')
        ################
        # pack all
        self.canvas.pack()
